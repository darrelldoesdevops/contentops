# Requirements Archive: v1.1 Polish & Pipeline
...
- [x] **DOCT-01**: User can run `contentops doctor` to check all prerequisites
- [x] **DOCT-02**: Doctor checks ffmpeg, ffprobe, whisper-cli, and claude on PATH with colored pass/warn/fail
- [x] **DOCT-03**: Doctor shows per-subcommand readiness summary
- [x] **DOCT-04**: Doctor checks minimum ffmpeg version (>= 6.0)
- [x] **DOCT-05**: Doctor exits 0 by default, exits 1 with `--strict`
- [x] **DOCT-06**: Commands auto-suggest `contentops doctor` when failing due to missing prerequisite
- [x] **DOCT-07**: `require_claude()` added and enforced in overlay `--auto` path

### Pipeline

- [x] **PIPE-01**: User can run `contentops pipeline input.mp4` to chain cut, caption, and overlay
- [x] **PIPE-02**: Pipeline manages intermediate files in a temp directory, not working directory
- [x] **PIPE-03**: Pipeline preserves temp directory on failure with path and recovery hint
- [x] **PIPE-04**: Pipeline supports `--dry-run` showing planned stages without executing
- [x] **PIPE-05**: Pipeline accepts `--model` flag for whisper model path

### CI/CD

- [x] **CICD-01**: GitHub Actions runs fmt, clippy, and tests on push/PR
- [x] **CICD-02**: Tag-triggered release builds ARM64 and x86_64 macOS binaries
- [x] **CICD-03**: Release includes universal macOS binary via lipo
- [x] **CICD-04**: Release artifacts include SHA256 checksums
- [x] **CICD-05**: CI pipeline includes cargo-audit security check
...
| DOCT-01 | Phase 7 | Complete |
| DOCT-02 | Phase 7 | Complete |
| DOCT-03 | Phase 7 | Complete |
| DOCT-04 | Phase 7 | Complete |
| DOCT-05 | Phase 7 | Complete |
| DOCT-06 | Phase 7 | Complete |
| DOCT-07 | Phase 7 | Complete |
